...
class _BrandsScreenState extends ConsumerState<BrandsScreen>
  ...
  Widget build(BuildContext context) {
    ...
    return brandsAsync.when(
      ...
      error: (_, __) => Scaffold(
        body: Center(child: Text('تعذر تحميل البراندات', style: AppTextStyles.body())),
      ),
      data: (all) => _buildList(all),
    );
  }

  Widget _buildList(List<BrandModel> allBrands) {
    final filtered = _filtered(allBrands);
    final featured = allBrands.where((b) => b.isFeatured).toList();
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: CustomScrollView(
          ...
          slivers: [
            SliverToBoxAdapter(
              child: _Header(count: allBrands.length),
            ),
            SliverToBoxAdapter(
              child: _SearchField(
