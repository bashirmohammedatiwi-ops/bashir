...
class _CartScreenState extends ConsumerState<CartScreen> {
  ...
  Future<void> _applyCoupon() async {
    ...
    try {
      final result = await ref.read(storeApiProvider).validateCoupon(code);
      if (result == null) {
        if (mounted) {
      ...
      final discount = (result['discount'] as num?)?.toInt() ??
          (result['discountAmount'] as num?)?.toInt() ??
          0;
      final percent = (result['percent'] as num?)?.toInt();
      setState(() {
        _appliedCoupon = code;
        _couponDiscount = percent != null
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('تعذر التحقق من الكود')),
        );
      }
    }
  }
