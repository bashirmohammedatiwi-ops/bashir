...
function renderBrandsPanels() {
  ...
  if ($('#sidebarBrands')) renderBrandsGrid(brands, { ...opts, root: $('#sidebarBrands') });
  if ($('#welcomeBrands')) {
    renderBrandsGrid(brands.slice(0, 24), { ...opts, root: $('#welcomeBrands') });
...
function barcodeMetaRows(product, shade = null) {
  ...
  const categoryBi = product.category || product.categoryEn
    ...
    : '';
  const storeLinks = [
    product.productUrl ? `<a href="${esc(product.productUrl)}" target="_blank" rel="noopener">elryan.com/ar ↗</a>` : '',
    product.productUrlEn ? `<a href="${esc(product.productUrlEn)}" target="_blank" rel="noopener">elryan.com/en ↗</a>` : '',
  ].filter(Boolean).join(' · ');
  return [
    metaRow('رقم المنتج', product.id),
    shade ? metaRow('المتغير', biText(shade.name, shade.nameEn, { block: false })) : '',
    ...
    storeLinks ? metaRow('رابط الريان', storeLinks, 'data-link') : '',
  ].join('');
}
...
function renderShadeSwatch(shade) {
  ...
  if (shade.image) {
    return `<span class="shade-color-box${oos ? ' shade-oos' : ''}" title="${esc(shade.name || '')}">
      <img src="${esc(shade.image)}" alt="${esc(shade.name || '')}" referrerpolicy="no-referrer" />
    </span>`;
  }
...
function renderShadeCard(s, active) {
  const nameHtml = biText(s.name, s.nameEn, { block: false });
  return `
...
function applyShadeSelection(product, shades, shade) {
  ...
  $('#shadeSelectedName').innerHTML = biText(
    [shade.name, displayBarcode(shade), shade.sku].filter(Boolean).join(' · '),
    [shade.nameEn, displayBarcode(shade), shade.sku].filter(Boolean).join(' · '),
    { block: false },
  ) || '—';
...
async function init() {
  ...
  bindSortPills($('#sortPills'), {
    ...
    onSort: (sort) => {
      state.sort = sort;
      if (state.products.length && state.sort !== 'default') {
        updateUI();
