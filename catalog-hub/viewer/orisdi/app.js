...
function displayOrisdiRef(item) {
...
function barcodeMetaRows(product, shade = null) {
  ...
  const orisdiRef = displayOrisdiRef(target) || displayOrisdiRef(product);
  const rows = [
  ...
  if (orisdiRef && orisdiRef !== barcode) {
    rows.push(metaRow('معرّف Orisdi', orisdiRef, 'data-orisdi-ref'));
  }
  rows.push(
...
function shadeStatsLabel(shades = []) {
  const withBarcode = shades.filter((s) => displayBarcode(s) || displayOrisdiRef(s)).length;
...
function applyShadeSelection(product, shades, shade) {
  ...
  if (selectedLabel) {
    selectedLabel.textContent = [shade.name, displayBarcode(shade) || displayOrisdiRef(shade), shade.sku].filter(Boolean).join(' · ') || '—';
  }
  ...
  if (detail) {
    ...
    const orisdiRef = displayOrisdiRef(shade) || displayOrisdiRef(product);
    detail.innerHTML = `
      <div class="shade-detail">
        ...
        <div class="shade-detail-text">
          ...
          ${orisdiRef && orisdiRef !== barcode ? `<div>معرّف Orisdi: <code dir="ltr">${esc(orisdiRef)}</code></div>` : ''}
