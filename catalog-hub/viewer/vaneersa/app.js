...
function renderCategoryTree() {
  ...
  function renderNode(node) {
    ...
    const link = `<a class="cat-link${node.isLeaf ? ' leaf' : ' parent'}${active}" data-id="${esc(node.slug)}" data-path="${esc(node.path)}" title="${esc(node.nameEn && node.nameEn !== node.name ? node.nameEn : '')}">${esc(node.name)}${node.nameEn && node.nameEn !== node.name ? `<span class="cat-en"> / ${esc(node.nameEn)}</span>` : ''}${count}</a>`;
...
function displayVaneersaRef(item) {
...
function barcodeMetaRows(product, shade = null) {
  ...
  const vaneersaRef = displayVaneersaRef(target) || displayVaneersaRef(product);
  const rows = [
  ...
  if (vaneersaRef && vaneersaRef !== barcode) {
    rows.push(metaRow('معرّف Vaneersa', vaneersaRef, 'data-vaneersa-ref'));
  }
  rows.push(
...
function shadeStatsLabel(shades = []) {
  const withBarcode = shades.filter((s) => displayBarcode(s) || displayVaneersaRef(s)).length;
...
function applyShadeSelection(product, shades, shade) {
  ...
  if (selectedLabel) {
    selectedLabel.textContent = [shade.name, displayBarcode(shade) || displayVaneersaRef(shade), shade.sku].filter(Boolean).join(' · ') || '—';
  }
  ...
  if (detail) {
    ...
    const vaneersaRef = displayVaneersaRef(shade) || displayVaneersaRef(product);
    detail.innerHTML = `
      <div class="shade-detail">
        ...
        <div class="shade-detail-text">
          ...
          ${vaneersaRef && vaneersaRef !== barcode ? `<div>معرّف Vaneersa: <code dir="ltr">${esc(vaneersaRef)}</code></div>` : ''}
...
function renderProductPanel(product, shades) {
  ...
  return `
    ...
    ${product.nameEn && product.nameEn !== product.name ? `<div class="p-title-en" dir="ltr">${esc(product.nameEn)}</div>` : ''}
    <div class="p-brand">${esc(product.manufacturer)}${product.manufacturerEn && product.manufacturerEn !== product.manufacturer ? ` · <span dir="ltr">${esc(product.manufacturerEn)}</span>` : ''}</div>
