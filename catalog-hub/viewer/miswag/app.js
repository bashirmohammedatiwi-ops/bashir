...
function displayMiswagRef(item) {
  return String(item?.miswagRef ?? '').trim();
}
...
function barcodeMetaRows(product, shade = null) {
  ...
  const miswagRef = displayMiswagRef(target) || displayMiswagRef(product);
  const rows = [
  ...
  if (barcode) {
    ...
    rows.push(metaRow('الباركود', 'غير متوفر في API Vanilla'));
  }
  if (miswagRef && miswagRef !== barcode) {
    rows.push(metaRow('معرّف Miswag', miswagRef, 'data-miswag-ref'));
  }
  rows.push(
...
function shadeStatsLabel(shades = []) {
  const withBarcode = shades.filter((s) => displayBarcode(s) || displayMiswagRef(s)).length;
...
function applyShadeSelection(product, shades, shade) {
  ...
  if (selectedLabel) {
    selectedLabel.textContent = [shade.name, displayBarcode(shade) || displayMiswagRef(shade), shade.sku].filter(Boolean).join(' · ') || '—';
  }
  ...
  if (detail) {
    ...
    const miswagRef = displayMiswagRef(shade) || displayMiswagRef(product);
    detail.innerHTML = `
      <div class="shade-detail">
        ...
        <div class="shade-detail-text">
          ...
          ${miswagRef && miswagRef !== barcode ? `<div>معرّف Miswag: <code dir="ltr">${esc(miswagRef)}</code></div>` : ''}
