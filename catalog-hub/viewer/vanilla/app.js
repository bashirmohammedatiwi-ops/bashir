...
function displayVanillaRef(item) {
  return String(item?.vanillaRef ?? '').trim();
}
...
function barcodeMetaRows(product, shade = null) {
  ...
  const vanillaRef = displayVanillaRef(target) || displayVanillaRef(product);
  const rows = [
  ...
  if (vanillaRef && vanillaRef !== barcode) {
    rows.push(metaRow('مرجع Vanilla', vanillaRef, 'data-vanilla-ref'));
  }
  rows.push(
...
function shadeStatsLabel(shades = []) {
  const withBarcode = shades.filter((s) => displayBarcode(s) || displayVanillaRef(s)).length;
...
function applyShadeSelection(product, shades, shade) {
  ...
  if (selectedLabel) {
    selectedLabel.textContent = [shade.name, displayBarcode(shade) || displayVanillaRef(shade), shade.sku].filter(Boolean).join(' · ') || '—';
  }
  ...
  if (detail) {
    ...
    const vanillaRef = displayVanillaRef(shade) || displayVanillaRef(product);
    detail.innerHTML = `
      <div class="shade-detail">
        ...
        <div class="shade-detail-text">
          ...
          ${vanillaRef && vanillaRef !== barcode ? `<div>مرجع Vanilla: <code dir="ltr">${esc(vanillaRef)}</code></div>` : ''}
