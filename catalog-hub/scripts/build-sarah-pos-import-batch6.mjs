#!/usr/bin/env node
/** Build batch-6 — 50 Sarah+POS products with auto categories + EN copy. */
...
const candidates = JSON.parse(readFileSync(new URL('../data/sarah-pos-candidates-batch6.json', import.meta.url), 'utf8'));
...
/** Manual verified metadata for all batch-6 products */
const META = JSON.parse(readFileSync(new URL('../data/sarah-pos-batch6-meta.json', import.meta.url), 'utf8'));
...
writeFileSync(new URL('../data/sarah-pos-import-products-batch6.json', import.meta.url).pathname, `${JSON.stringify(selected, null, 2)}\n`);
console.log(`Built batch6: ${selected.length} products`);
