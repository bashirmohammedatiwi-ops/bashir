 ...
 * - الاسم العربي كاملاً بالعربية: البراند - المنتج
 * - الوصف من وجوه بالعربية والإنجليزية
 * - تصنيف يدوي حسب قسم وجوه
 *
 * Usage:
 *   LIMIT=50 MIN_POS_STOCK=2 node scripts/import-faces-pos-batch25.mjs
 *   SCAN_ONLY=1 node scripts/import-faces-pos-batch25.mjs
 */
...
const SEARCH_MAX_PAGES = Number(process.env.SEARCH_MAX_PAGES || 5);
...
const STATE_FILE = path.join(__dirname, '../data/faces-pos-batch25-state.json');
const CANDIDATES_FILE = path.join(__dirname, '../data/faces-pos-batch25-candidates.json');
...
const FACES_SEARCH = [
  ...
  'cerave', 'la roche posay', 'huda beauty', 'sol de janeiro', 'mancera', 'armaf',
  'essence', 'catrice', 'sheglam', 'kerastase', 'cosrx', 'skin1004', 'nars', 'mac',
  'gucci', 'prada', 'versace', 'burberry', 'givenchy', 'valentino', 'chloe', 'rabanne',
  'dolce gabbana', 'issey miyake', 'ralph lauren', 'kilian', 'byredo', 'amouage',
  'montale', 'xerjoff', 'jean paul gaultier', 'carolina herrera', 'calvin klein',
  'hugo boss', 'benefit', 'bobbi brown', 'charlotte tilbury', 'urban decay', 'maybelline',
  'anua', 'laneige', 'bioderma', 'vichy', 'eucerin', 'the ordinary', 'shiseido',
  'olaplex', 'moroccanoil', 'redken', 'clarins', 'estee lauder', 'guerlain', 'bvlgari',
  'عطر', 'مكياج', 'سيروم', 'شامبو', 'كريم', 'ماسكارا', 'أحمر شفاه',
];
...
function appendShadeName(names, shade) {
  return appendArabicShade(names, shade);
}
...
async function importOne(brandCache, row, state) {
  ...
  names = polishFacesArabicNames({
    ...names,
    productAr: names.productAr,
    productEn: names.productEn,
  });
  ...
  if (!isValidArabicName(names.nameAr) || names.validAr === false) {
...
async function tryImportRow(brandCache, row, state, existing) {
  ...
  if (importedBarcodes.has(row.barcode) || state.imported[row.barcode]) return;
...
async function drainPool(brandCache, pool, state, existing) {
  const unique = [...new Map(pool.map((c) => [c.barcode, c])).values()]
...
async function main() {
  ...
  console.log(`FACES import batch25`);
  ...
  if (existsSync(CANDIDATES_FILE)) {
    try {
      ...
      pushCandidates(candidates, saved);
      console.log(`Loaded ${saved.length} saved candidates — importing immediately...\n`);
      await drainPool(brandCache, candidates, state, existing);
    } catch { /* ignore */ }
  }

  if (SCAN_ONLY) {
    if (stats.ok >= LIMIT) {
      console.log(`\nTarget reached (${stats.ok}) — SCAN_ONLY stop.`);
      saveState(state);
      return;
    }
  }

  if (!IMPORT_ONLY && stats.ok < LIMIT) {
    if (USE_SEARCH) {
      const searchHits = await discoverFromSearch(existing, excluded);
      pushCandidates(candidates, searchHits);
      console.log(`Search: ${searchHits.length} hits (pool ${candidates.length})`);
      await drainPool(brandCache, candidates, state, existing);
    }
    ...
    if (stats.ok < LIMIT) {
      ...
      for (const leaf of leaves) {
        ...
        if (rows.length) {
          console.log(`  scan ${leaf}: ${rows.length} POS hits`);
          pushCandidates(candidates, rows);
          await drainPool(brandCache, rows, state, existing);
        }
      }
    }
  } else if (IMPORT_ONLY && stats.ok < LIMIT) {
    await drainPool(brandCache, candidates, state, existing);
  }
  ...
  console.log(`Candidates saved: ${unique.length} → ${CANDIDATES_FILE}`);
