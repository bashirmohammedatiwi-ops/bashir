#!/usr/bin/env node
/** Polish batch-6 — verified EN names, Sarah AR names, bilingual descriptions. */
import { readFileSync, writeFileSync } from 'fs';
import { CATEGORIES } from '../lib/core/app-categories.js';

const PRODUCTS = new URL('../data/sarah-pos-import-products-batch6.json', import.meta.url).pathname;
const CANDIDATES = new URL('../data/sarah-pos-candidates-batch6.json', import.meta.url).pathname;
...
const DESC_AR = JSON.parse(readFileSync(new URL('../data/sarah-pos-batch6-desc-ar.json', import.meta.url), 'utf8'));
...
for (const p of products) {
  ...
  if (meta) {
    ...
    if (meta.nameAr) p.nameAr = meta.nameAr;
...
console.log(`Polished ${products.length} batch6 products`);
