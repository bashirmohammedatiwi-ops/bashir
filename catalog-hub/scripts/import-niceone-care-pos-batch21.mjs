#!/usr/bin/env node
/**
 * استيراد 50 منتج عناية من نايس وان:
 * - موجود في POS ومخزونه 2 أو أكثر
 * - بدون تدرجات في التطبيق (باركود POS واحد)
 * - الاسم: البراند + المنتج (عربي وإنجليزي) من نايس وان
 * - الوصف من نايس وان بالعربية والإنجليزية
 * - تصنيف يدوي حسب قسم نايس وان
 *
 * Usage:
 *   LIMIT=50 MIN_POS_STOCK=2 node scripts/import-niceone-care-pos-batch21.mjs
 *   SCAN_ONLY=1 node scripts/import-niceone-care-pos-batch21.mjs
 */
...
import {
  ...
  isCareDetail,
  inferCareTypeKey,
} from '../lib/core/care-product-filter.js';
...
const USE_INDEX = process.env.USE_INDEX !== '0';
const USE_SEARCH = process.env.USE_SEARCH !== '0';
const IMPORT_ONLY = process.env.IMPORT_ONLY === '1';
const STATE_FILE = path.join(__dirname, '../data/niceone-care-pos-batch21-state.json');
const CANDIDATES_FILE = path.join(__dirname, '../data/niceone-care-pos-batch21-candidates.json');

const catsJson = JSON.parse(readFileSync(path.join(__dirname, '../data/niceone-categories.json'), 'utf8'));
const SCAN_LEAVES = collectCareLeaves(catsJson);

const PERF = /parfum|perfume|eau de|edp|edt|cologne|عطر|fragrance/i;
const SKIP_NAME = /tester|sample|تستر|عينة|\btest\b/i;

const CARE_SEARCH = [
  'cerave', 'la roche posay', 'eucerin', 'vichy', 'bioderma', 'the ordinary', 'cosrx', 'anua',
  'laneige', 'garnier', 'nivea', 'dove', 'pantene', 'head shoulders', 'tresemme', 'kerastase',
  'olaplex', 'cantu', 'shea moisture', 'neutrogena', 'cetaphil', 'aveeno', 'mustela', 'uriage',
  'avene', 'isdin', 'some by mi', 'medicube', 'skin1004', 'round lab', 'isntree', 'beauty of joseon',
  'panoxyl', 'embryolisse', 'differin', 'sensodyne', 'colgate', 'listerine', 'dettol', 'vaseline',
  'سيروم', 'كريم', 'غسول', 'تونر', 'شامبو', 'بلسم', 'واقي شمس', 'مزيل عرق', 'معجون اسنان',
  'غسول فم', 'مرطب', 'ماسك', 'علاج شعر', 'عناية الوجه', 'عناية الشعر',
];
...
function isPerfume(text = '') {
  return PERF.test(text) && !/body|hair|skin|care|deodorant|shampoo|cream|lotion|micellar/i.test(text);
}
...
function resolveShadeImport(detail, barcode, posName = '') {
  ...
  if (detail.barcode && gtinEqual(detail.barcode, barcode) && shades.length > 1) {
...
async function importOne(brandCache, row, state) {
  ...
  if (!isCareDetail(detail, leaf)) return { status: 'skip', reason: 'not care' };
  if (isPerfume(`${detail.nameAr} ${detail.nameEn} ${pos?.pos?.name || ''}`)) {
  ...
  const typeKey = inferCareTypeKey(`${names.nameEn} ${names.nameAr}`, leaf);
  const cats = resolveCareCategories(leaf, {
    barcode,
    typeKey,
    brandEn: names.brandEn,
    brandAr: names.brandAr,
    posName: pos?.pos?.name || '',
  });
...
async function posFilterListingItems(items, leaf, existing, excluded) {
  ...
  for (const it of items) {
    ...
    if (!isCareCandidate({ leaf, nameAr: it.nameAr, nameEn: it.nameEn })) continue;
...
async function discoverFromIndex(existing, excluded) {
  ...
  for (const e of entries) {
    ...
    seen.add(bc);
    rows.push({ barcode: bc, niceoneId: String(e.productId), leaf: 'care' });
  }
...
async function discoverFromSearch(existing, excluded) {
  ...
  for (const query of CARE_SEARCH) {
    for (let page = 1; page <= 3; page++) {
      const listing = await searchProducts(query, { page, limit: LIST_LIMIT });
      const hits = await posFilterListingItems(listing.items || [], 'care', existing, excluded);
      for (const h of hits) {
...
async function main() {
  ...
  console.log(`Nice One care import batch21`);
  ...
  for (const row of unique) {
    ...
    try {
      ...
      if (r.status === 'ok') {
        ...
        console.log(`OK ${row.barcode} | ${r.nameAr?.slice(0, 55)} | stock=${r.stock} | ${r.typeKey}`);
      } else {
        stats.skip += 1;
