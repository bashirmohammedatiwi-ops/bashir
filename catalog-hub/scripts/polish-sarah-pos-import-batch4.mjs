#!/usr/bin/env node
/** Polish batch-3 — verified EN names, Sarah AR names, bilingual descriptions. */
import { readFileSync, writeFileSync } from 'fs';
import { CATEGORIES } from '../lib/core/app-categories.js';

const PRODUCTS = new URL('../data/sarah-pos-import-products-batch4.json', import.meta.url).pathname;
const CANDIDATES = new URL('../data/sarah-pos-candidates-batch4.json', import.meta.url).pathname;
...
const META = JSON.parse(readFileSync(new URL('../data/sarah-pos-batch4-meta.json', import.meta.url), 'utf8'));
const DESC_AR = JSON.parse(readFileSync(new URL('../data/sarah-pos-batch4-desc-ar.json', import.meta.url), 'utf8'));
...
for (const p of products) {
  ...
  if (meta) { p.brandEn = meta.brandEn; p.nameEn = meta.nameEn; }
  if (src?.nameAr) p.nameAr = nameAr(src.nameAr);
...
console.log(`Polished ${products.length} batch4 products`);
