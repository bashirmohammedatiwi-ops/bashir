 ...
 *
 * Usage:
 *   LIMIT=50 MIN_POS_STOCK=2 node scripts/import-faces-pos-batch22.mjs
 *   SCAN_ONLY=1 node scripts/import-faces-pos-batch22.mjs
 */
...
const STATE_FILE = path.join(__dirname, '../data/faces-pos-batch22-state.json');
const CANDIDATES_FILE = path.join(__dirname, '../data/faces-pos-batch22-candidates.json');
...
async function main() {
  ...
  console.log(`FACES import batch22`);
  ...
  for (const row of unique) {
    ...
    try {
      ...
      if (r.status === 'ok') {
        ...
        console.log(`OK ${row.barcode} | ${r.nameAr?.slice(0, 55)} | stock=${r.stock} | ${r.kind}`);
      } else {
        stats.skip += 1;
