#!/usr/bin/env node
/**
 * استيراد 50 منتج بريميوم من نايس وان:
 ...
 *
 * Usage:
 *   LIMIT=50 MIN_POS_STOCK=2 node scripts/import-niceone-premium-pos-batch25.mjs
 */
...
import { collectPremiumLeaves, resolvePremiumCategories } from '../lib/core/premium-category-map.js';
import {
  applyPremiumMeta,
...
const STATE_FILE = path.join(__dirname, '../data/niceone-premium-pos-batch25-state.json');
const CANDIDATES_FILE = path.join(__dirname, '../data/niceone-premium-pos-batch25-candidates.json');
...
async function importOne(brandCache, row, state) {
  ...
  const desc = buildPremiumDescriptions(metaOverride || applied.meta, storeDescAr, storeDescEn);
  ...
  if (metaOverride?.lipType === 'lipTint') {
    // بريميوم: القسم الثانوي تحت مكياج فاخر فقط (لا tertiary من شجرة المكياج)
...
async function main() {
  ...
  console.log(`Nice One PREMIUM import batch25`);
  console.log(`Leaves: ${SCAN_LEAVES.join(', ')}`);
  console.log(`Min POS stock: ${MIN_POS_STOCK}+ | target: ${LIMIT}`);
