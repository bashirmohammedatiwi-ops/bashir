#!/usr/bin/env node
/** Build batch-7 — 50 Sarah+POS products with auto categories + EN copy. */
...
const candidates = JSON.parse(readFileSync(new URL('../data/sarah-pos-candidates-batch7.json', import.meta.url), 'utf8'));
...
/** Manual verified metadata for all batch-7 products */
const META = JSON.parse(readFileSync(new URL('../data/sarah-pos-batch7-meta.json', import.meta.url), 'utf8'));
...
const SELECTED = Object.keys(JSON.parse(readFileSync(new URL('../data/sarah-pos-batch7-meta.json', import.meta.url), 'utf8')));
...
writeFileSync(new URL('../data/sarah-pos-import-products-batch7.json', import.meta.url).pathname, `${JSON.stringify(selected, null, 2)}\n`);
console.log(`Built batch7: ${selected.length} products`);
