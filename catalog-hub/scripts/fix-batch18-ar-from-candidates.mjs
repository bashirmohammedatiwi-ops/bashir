...
const cands = JSON.parse(readFileSync(new URL(process.env.CANDIDATES_FILE || '../data/sarah-pos-candidates-care-batch18.json', import.meta.url), 'utf8'));
...
for (const [bc, m] of Object.entries(meta)) {
  ...
  const productAr = cleanProductAr(row?.nameAr || row?.posName || m.nameEn, brandAr);
...
writeFileSync(new URL('../data/sarah-pos-batch18-meta.json', import.meta.url).pathname, `${JSON.stringify(meta, null, 2)}\n`);
let bad = 0;
