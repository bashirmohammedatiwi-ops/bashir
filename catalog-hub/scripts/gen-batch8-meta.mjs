...
const OUT = new URL('../data/sarah-pos-batch8-meta.json', import.meta.url).pathname;
const ORDER_OUT = new URL('../data/sarah-pos-batch8-order.json', import.meta.url).pathname;

const SKIP = /تستر|tester|sample|كرتون|T\*|T\*L|T\*M|T\*S|T-L|T\*V|T\*M\*|T\*GUERLINE|T\*JPG/i;
...
const pool = CANDIDATES
  .filter((c) => !SKIP.test(`${c.nameAr || ''} ${c.posName || ''}`))
...
writeFileSync(new URL('../data/sarah-pos-batch8-desc-ar.json', import.meta.url).pathname, '{}\n');
