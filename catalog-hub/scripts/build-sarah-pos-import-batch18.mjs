#!/usr/bin/env node
/** Build batch-8 — 50 Sarah+POS products with auto categories + EN copy. */
...
const imported = new Set([
  ...
  ...Object.keys(JSON.parse(readFileSync(new URL('../data/sarah-pos-import-state-batch7.json', import.meta.url), 'utf8')).imported || {}),
  ...Object.keys(JSON.parse(readFileSync(new URL('../data/sarah-pos-import-state-batch8.json', import.meta.url), 'utf8')).imported || {}),
  ...Object.keys(JSON.parse(readFileSync(new URL('../data/sarah-pos-import-state-batch9.json', import.meta.url), 'utf8')).imported || {}),
  ...Object.keys(JSON.parse(readFileSync(new URL('../data/sarah-pos-import-state-batch10.json', import.meta.url), 'utf8')).imported || {}),
  ...Object.keys(JSON.parse(readFileSync(new URL('../data/sarah-pos-import-state-batch11.json', import.meta.url), 'utf8')).imported || {}),
  ...Object.keys(JSON.parse(readFileSync(new URL('../data/sarah-pos-import-state-batch12.json', import.meta.url), 'utf8')).imported || {}),
  ...Object.keys(JSON.parse(readFileSync(new URL('../data/sarah-pos-import-state-batch13.json', import.meta.url), 'utf8')).imported || {}),
  ...Object.keys(JSON.parse(readFileSync(new URL('../data/sarah-pos-import-state-batch14.json', import.meta.url), 'utf8')).imported || {}),
  ...Object.keys(JSON.parse(readFileSync(new URL('../data/sarah-pos-import-state-batch15.json', import.meta.url), 'utf8')).imported || {}),
  ...Object.keys(JSON.parse(readFileSync(new URL('../data/sarah-pos-import-state-batch16.json', import.meta.url), 'utf8')).imported || {}),
  ...Object.keys(JSON.parse(readFileSync(new URL('../data/sarah-pos-import-state-batch17.json', import.meta.url), 'utf8')).imported || {}),
]);
const CAND_FILE = process.env.CANDIDATES_FILE || '../data/sarah-pos-candidates-care-batch18.json';
const candidates = JSON.parse(readFileSync(new URL(CAND_FILE, import.meta.url), 'utf8'));
...
/** Manual verified metadata for all batch-8 products */
...
function build(row) {
  ...
  if (override) {
    const { kind, subs, careLeaf, typeKey, makeupSub, brandEn, nameEn, nameAr, brandAr, descriptionEn, descriptionAr, isNew: metaIsNew } = override;
    const base = { barcode: row.barcode, sarahId: row.sarahId, url: row.url, stock: row.stock, brandEn, nameEn };
    if (nameAr) base.nameAr = nameAr;
    if (brandAr) base.brandAr = brandAr;
    if (descriptionEn && descriptionAr) {
      base.descriptionEn = descriptionEn;
      base.descriptionAr = descriptionAr;
    }
    if (kind === 'care') {
      ...
      return {
        ...
        isNew: !!metaIsNew,
        ...(descriptionEn && descriptionAr ? {} : cEn({ intro: `${nameEn} supports daily care with a trusted formula.`, cat: 'Skincare', type: typeKey?.replace(/-/g, ' ') || 'care', benefits: ['Daily care', 'Trusted formula', 'Regular use'], size })),
      };
    }
    if (kind === 'makeup') {
      ...
      return {
        ...
        tertiaryCategoryIds: [],
        isNew: !!metaIsNew,
        ...(descriptionEn && descriptionAr ? {} : mEn({ intro: `${nameEn} delivers reliable makeup performance for everyday looks.`, type: makeupSub === 'eyes' ? 'Eye makeup' : makeupSub === 'lips' ? 'Lip makeup' : 'Face makeup', benefits: ['Easy application', 'Buildable result', 'Everyday wear'] })),
      };
    }
    ...
    return {
      ...
      tertiaryCategoryIds: [],
      isNew: metaIsNew ?? !!subs?.isNew,
      ...(descriptionEn && descriptionAr ? {} : pEn({ intro: `${nameEn} is a refined fragrance with elegant character and lasting presence.`, family: 'Eau de parfum', notes: 'Bergamot, florals, amber, woods, musk', character: 'Elegant and long-lasting', best: 'Daily to evening wear', long: '6–9 hours with good projection' })),
    };
  }
...
const SELECTED = Object.keys(JSON.parse(readFileSync(new URL('../data/sarah-pos-batch18-meta.json', import.meta.url), 'utf8')));
const byBcCand = new Map(candidates.map((c) => [c.barcode, c]));

const selected = [];
for (const bc of SELECTED) {
  const row = byBcCand.get(bc);
  if (!row || SKIP.has(bc) || imported.has(bc)) {
    console.warn(`Skip missing/excluded: ${bc}`);
    continue;
  }
  selected.push(build(row));
}

writeFileSync(new URL('../data/sarah-pos-import-products-batch18.json', import.meta.url).pathname, `${JSON.stringify(selected, null, 2)}\n`);
console.log(`Built batch18: ${selected.length} products`);
