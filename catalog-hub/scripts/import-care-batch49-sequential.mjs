#!/usr/bin/env node
/**
 * Import care batch49 products one barcode at a time with verification.
 * Usage: node scripts/import-care-batch49-sequential.mjs
...
const PRODUCTS_FILE = path.join(__dirname, '../data/care-batch49-products.json');
...
async function main() {
  ...
  console.log(`Batch49 sequential import | barcodes=${TARGET.length} | delay=${DELAY_MS}ms | dry=${DRY_RUN ? 'yes' : 'no'}\n`);
  ...
  for (let i = 0; i < TARGET.length; i += 1) {
    ...
    if (hit?.inApp?.id || state.imported[barcode]) {
      stats.skip += 1;
      state.skipped[barcode] = { reason: 'already in app', at: Date.now() };
      saveState(state);
