...
const PERFUME = /عطر|perfume|parfum|eau de|edt|edp|edc|cologne|\boud\b|برفيوم|تواليت|بارفيوم/i;
function isPerfume(row) {
  if (row.category === 'care/makeup' || row.category === 'non-perfume') {
    const t = `${row.nameAr || ''} ${row.posName || ''}`;
    if (/عطر|perfume|parfum|eau de|edt|edp|edc|cologne|\boud\b|برفيوم|تواليت|بارفيوم/i.test(t)
      && !/معطر شعر|hair mist|deodorant|مزيل|شامبو|كريم|ماسكر|باليت|غسول|سيروم|مرطب|body mist|mask|lip|makeup|mascara|foundation|shampoo|conditioner|gel|lotion|cream|sunscreen|spf|gummy|vitamin|cleans|toner/i.test(t)) return true;
...
const OUT = new URL('../data/sarah-pos-batch18-meta.json', import.meta.url).pathname;
const ORDER_OUT = new URL('../data/sarah-pos-batch18-order.json', import.meta.url).pathname;
...
const pool = CANDIDATES.filter((c) => !SKIP.test(`${c.nameAr} ${c.posName}`) && !isPerfume(c))
...
for (const row of selected) {
  ...
  const productAr = cleanArName(row.nameAr || row.posName || row.nameEn || row.slugHint, brandAr);
  const nameAr = productAr ? `${brandAr} - ${productAr}` : brandAr;
  let nameEn = posToNameEn(row.posName, brandEn);
  if (!nameEn) nameEn = `${brandEn} ${productAr || row.nameEn || row.slugHint || ''}`.trim().slice(0, 120);
...
writeFileSync(new URL('../data/sarah-pos-batch18-desc-ar.json', import.meta.url).pathname, '{}\n');
