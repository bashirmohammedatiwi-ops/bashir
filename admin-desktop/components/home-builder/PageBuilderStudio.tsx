...
export type CmsPageKey = "HOME" | "OFFERS";

export type PageBuilderStudioProps = {
  pageKey: CmsPageKey;
  title: string;
  subtitle: string;
...
export function PageBuilderStudio({
  ...
  const builderBlocks = useMemo(
    () => (isHomePage ? filterBuilderBlocks(sorted) : sorted),
    [sorted, isHomePage],
  );
  ...
  function invalidateBlocks() {
    return qc.invalidateQueries({ queryKey: [blocksQueryKey] });
  }
  ...
  const applyTemplate = useMutation({
    mutationFn: async ({ templateId, replace }: { templateId: string; replace?: boolean }) => {
      ...
      const cmsSections = isHomePage
        ? tpl.sections.filter((s) => s.type !== "HERO_BANNER")
        : tpl.sections;
  ...
  const importBlocks = useMutation({
    mutationFn: async (json: string) => {
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        if (isHomePage && isFixedTopSection(type)) continue;
  ...
  function startCreate(type: SectionType, preset?: { title?: string; subtitle?: string; payload?: Record<string, unknown> }) {
    if (isHomePage && isFixedTopSection(type)) {
  ...
  function exportSelectedJson() {
    ...
    a.href = url;
    a.download = `${exportFilePrefix}-sections-selected.json`;
    a.click();
  ...
  function exportJson() {
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `${exportFilePrefix}-${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
    URL.revokeObjectURL(url);
