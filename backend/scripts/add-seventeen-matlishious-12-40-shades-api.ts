/**
 * Add missing Seventeen Matlishious Super Stay Lip Color shades 12 & 40.
 * Preserves existing shade barcodes — does NOT delete them.
 * New shades have NO barcodes (catalog rule).
 *
 * Product: 9eebc6cf-1b7d-4450-8b4b-16585c084a16
 * Sources: seventeencosmetics.com (40), myoras (12 pack photo)
 * Usage: npx tsx scripts/add-seventeen-matlishious-12-40-shades-api.ts
 */
const API_BASE = (process.env.API_BASE ?? "https://deemaalhayat.com/api/v1").replace(/\/$/, "");
const ADMIN_EMAIL = process.env.ADMIN_EMAIL ?? "admin@alhayaa.com";
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD ?? "000000";

const PRODUCT_ID = "9eebc6cf-1b7d-4450-8b4b-16585c084a16";
...
/** Shade codes only (match existing Matlishious naming). Hex sampled from pack photos. */
const NEW_SHADES: ShadeInput[] = [
  {
    name: "12",
    colorHex: "#902C3B",
    imageUrl:
      "https://cdn.shopify.com/s/files/1/0625/2537/4676/files/seventeen-matlishious-super-stay-lip-color-oras-official-12.jpg?v=1689772528",
  },
  {
    name: "40",
    colorHex: "#E78795",
    imageUrl: `${OFF}/2025/10/matlishious_40.png`,
  },
];
...
function patchDescriptions(product: Product, totalShades: number) {
  ...
  if (!/• 12[ —]/.test(descriptionAr) && !/• 12\n/.test(descriptionAr)) {
    descriptionAr = descriptionAr
      .replace(/\d+ درجة/, `${totalShades} درجة`)
      .replace("• 11 —", "• 11 —")
      .replace(/• 11[^\n]*\n/, (m) => m + "• 12 — أحمر عنابي متوسط\n");
    if (!/• 12/.test(descriptionAr)) descriptionAr += "\n• 12 — أحمر عنابي متوسط\n";
    if (!/• 40/.test(descriptionAr)) {
      descriptionAr = descriptionAr.replace(/• 28[^\n]*\n/, (m) => m + "• 40 — وردي فاتح ناعم\n");
      if (!/• 40/.test(descriptionAr)) descriptionAr += "• 40 — وردي فاتح ناعم\n";
    }
  }

  if (!/• 12[ —]/.test(descriptionEn) && !/• 12\n/.test(descriptionEn)) {
    descriptionEn = descriptionEn
      .replace(/\d+ shades/, `${totalShades} shades`)
      .replace(/\d+ official shades/, `${totalShades} official shades`);
    descriptionEn = descriptionEn.replace(/• 11[^\n]*\n/, (m) => m + "• 12 — medium berry red\n");
    if (!/• 12/.test(descriptionEn)) descriptionEn += "\n• 12 — medium berry red\n";
    if (!/• 40/.test(descriptionEn)) {
      descriptionEn = descriptionEn.replace(/• 28[^\n]*\n/, (m) => m + "• 40 — soft light pink\n");
      if (!/• 40/.test(descriptionEn)) descriptionEn += "• 40 — soft light pink\n";
    }
  }
...
async function main() {
  ...
  for (const shade of toAdd) {
    const imageId = await uploadImage(shade.imageUrl, `matlishious-${shade.name}`);
  ...
  for (const s of [...(after.shades ?? [])].sort((a, b) => a.position - b.position)) {
    ...
    console.log(
      `  ${String(s.position).padStart(2)} ${s.name.padEnd(6)} ${s.colorHex ?? ""}  ${s.barcode ?? "—"}${mark}`,
    );
  }
}
