/**
 * Add Essence Long-Lasting Eye Pencil shade 39 to existing product.
 * Source: https://www.essence.eu/en-om/p/941232/long-lasting-eye-pencil
 * Usage: npx tsx scripts/add-essence-long-lasting-eye-pencil-shade-39-api.ts
 */
const API_BASE = (process.env.API_BASE ?? "https://deemaalhayat.com/api/v1").replace(/\/$/, "");
const ADMIN_EMAIL = process.env.ADMIN_EMAIL ?? "admin@alhayaa.com";
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD ?? "000000";

const PRODUCT_ID = "ac89a445-da16-42d9-a91b-4bc121c1dd93";
...
const NEW_SHADES: ShadeInput[] = [
  {
    name: "39 Shimmer Sunsation",
    colorHex: "#F0A858",
    imageUrl:
      "https://static.beautytocare.com/cdn-cgi/image/f=auto/media/catalog/product/e/s/essence-longlasting-eye-pencil-39-shimmer-sunsation-0-28g.jpg",
    position: 9,
  },
];
...
async function main() {
  ...
  if (!toAdd.length) {
    console.log("Shade 39 already exists — nothing to add.");
    return;
  }

  console.log(`Adding: ${toAdd.map((s) => s.name).join(", ")}\n`);
  console.log("Uploading shade image...");
